# ESTE ARCHIVO ES EL PROPUESTO CON CAMBIOS EN MIGRATE
...
class TenantDatabaseRouter:
    DEFAULT_MODELS = ['empresadb', 'perfilusuario',
                      'movimientotimbresglobal','timbrescliente','sitemessages','articuloayuda'] 
    DEFAULT_APPS = ['auth', 'contenttypes', 'sessions', 'admin','timbres','ayuda']
    ...
    def allow_migrate(self, db, app_label, model_name=None, **hints):
        default_apps = {'auth', 'contenttypes', 'admin', 'sessions', 'timbres', 'ayuda'}
        tenant_apps = {'core', 'cxc', 'inv', 'fac'}  # solo operativas

        if db == 'default':
            # Default: auth/sessions/admin + modelos globales (incluye core si ahí migras EmpresaDB y PerfilUsuario)
            return app_label in (default_apps | {'core'})
        elif db == 'tenant':
            # Tenant: NO auth/sessions/admin/contenttypes
