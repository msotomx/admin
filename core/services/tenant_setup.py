...
# CREAR TENANT VERSION - ACTUALIZADA
def crear_tenant_completo_actualizado(request, nombre, password, nombre_comercial, email):
    # Paso 1: Normalizar nombre
    slug = slugify(nombre_comercial)
    slug = re.sub(r'[^a-z0-9_]', '', slug.replace('-', '_'))
    db_name = f'e_{slug}'
    ...
    if UserModel.objects.using('default').filter(username=email).exists():
        raise Exception(f"Ya existe un usuario registrado con username '{email}' ")
    ...
    return usuario_global, empresa_db
